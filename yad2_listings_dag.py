...
def yad2_listings_etl():
    ...
    def get_neighborhood_ids() -> List[int]:
        ...
        """
        try:
            # First try the original path from the script
            script_dir = os.path.dirname(os.path.abspath(__file__))
            backend_dir = os.path.dirname(script_dir)
            mapping_file_path = os.path.join(backend_dir, 'data', 'sources', 'yad2_hood_mapping.json')
            
            # If the file doesn't exist, try alternative paths or create a sample
            if not os.path.exists(mapping_file_path):
                # Try relative to current working directory
                alt_path = os.path.join('backend', 'data', 'sources', 'yad2_hood_mapping.json')
                if os.path.exists(alt_path):
                    mapping_file_path = alt_path
                else:
                    # Create a sample mapping file for demonstration
    ...
    def fetch_and_process_single_hood(hood_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Try to import populate_listings, or create a mock implementation
            try:
                from populate_database import populate_listings
            except ImportError:
                print("Warning: populate_database module not found. Using mock implementation.")
                
                def populate_listings(data):
                    """Mock implementation of populate_listings"""
                    print(f"Mock: Would populate {len(data.get('listings', []))} listings to database")
                    return {"status": "success", "count": len(data.get('listings', []))}
            ...
            # Transform: Enrich listings with additional attributes
            print(f"Enriching {len(parsed_results['listings'])} listings...")
            enriched_listings = enrich_listings(parsed_results['listings'])
    ...
    def generate_summary_report(results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        failed = [r for r in results if r.get('status') not in ['success', 'no_data', 'no_valid_data']]
        ...
        summary = {
            ...
            "failed_neighborhoods": len(failed),
            ...
            "failed_hood_ids": [r['hood_id'] for r in failed],
        ...
        print(f"- Failed: {summary['failed_neighborhoods']}")
        ...
        if summary['failed_hood_ids']:
            print(f"- Failed neighborhood IDs: {summary['failed_hood_ids']}")
    ...
    # Create dynamic parallel tasks for each neighborhood
    etl_results = [
        fetch_and_process_single_hood.override(task_id=f"etl_hood_{hood_id}")(hood_id)
        for hood_id in neighborhood_ids
    ]
    
    # Generate summary report after all ETL tasks complete
    summary = generate_summary_report(etl_results)
    
    # Set up dependencies - summary depends on all ETL tasks
